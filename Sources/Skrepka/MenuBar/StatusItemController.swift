...
/// and Skrepka needs both — the hotkey has to drive the same surface a click does.
@MainActor
final class StatusItemController {
    ...
    private func configureButton() {
        guard let button = statusItem.button else {
            SkrepkaLog.panel.error("Status item has no button; the menu bar icon will be missing.")
            return
        }
        button.image = StatusItemIcon.image()
        button.toolTip = "Skrepka — clipboard history"
    }
    ...
    func showProblem(_ problem: DiagnosticsProblem?) {
        ...
        statusItem.button?.toolTip = problem?.headline ?? "Skrepka — clipboard history"
    ...
    private func makeMenu() -> NSMenu {
        ...
        menu.addItem(.separator())
        menu.addItem(
            NSMenuItem(title: "Quit Skrepka", action: #selector(MenuActions.quit), keyEquivalent: "q")
        )
