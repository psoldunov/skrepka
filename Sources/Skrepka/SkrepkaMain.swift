...
/// Skrepka's entry point.
///
/// Plain AppKit rather than a SwiftUI `App`: the only windows are a menu bar
/// item, an `NSPanel` and a settings window, all of which Skrepka manages
...
enum SkrepkaMain {
    ...
    private static let delegate = SkrepkaAppDelegate()
...
final class SkrepkaAppDelegate: NSObject, NSApplicationDelegate {
