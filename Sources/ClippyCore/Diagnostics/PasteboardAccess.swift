...
///
/// A value type mirroring `NSPasteboard.AccessBehavior` so the rest of Clippy —
/// and its tests — need no live pasteboard. AppKit is imported here for the
/// enum only; `ClippyCore` still holds no view types.
public enum PasteboardAccess: Sendable, Hashable, CaseIterable {
    ...
    case alwaysAllow
    /// Reads are refused silently. Clippy sees an empty pasteboard forever.
    case alwaysDeny
