#!/usr/bin/env bash
#
# Builds Skrepka's Linux daemon, CLI and desktop app from this checkout and
...
# rewrite the unit and the launcher entries the same way, and uninstall the same
# way.
#
...
# The oldest GTK whose API the desktop app uses throughout:
...
usage() {
	cat << 'USAGE'
Builds skrepkad, skrepka and skrepka-gui from this checkout in release and
...
The desktop app — tray, picker and Settings — is built only when pkg-config
finds GTK 4.12 or newer and gtk4-layer-shell; without them the daemon and the
CLI install alone.
...
# The desktop app is the only product that links GTK4 and gtk4-layer-shell, so
# it is the only one whose build can fail on a machine that is otherwise fine. Building it is gated on both -dev packages being visible to pkg-config —
...
if command -v pkg-config > /dev/null 2>&1 \
	...
	# A failed build is a skipped desktop app, not a failed install: under
	# `set -e` a bare failure here would abort before the daemon and the CLI
	# built above were installed.
	if swift build -c release --product "${GUI_NAME}"; then
		BUILT_GUI=1
	else
		yellow "skipping ${GUI_NAME}: its build failed, see the output above."
		yellow "  The daemon and the CLI are installed regardless."
	fi
else
	yellow "skipping ${GUI_NAME}: pkg-config cannot find ${GTK_REQUIREMENT} and gtk4-layer-shell-0."
	yellow "  Install their development packages and re-run to get the tray, picker and Settings."
fi
...
#
# skrepka-gui is staged only if it was built in this run. A leftover binary
