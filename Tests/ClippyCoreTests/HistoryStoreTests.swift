...
struct HistoryStoreTests {
    ...
    func capturesEntry() throws {
        let store = try makeStore()
        #expect(store.capture(item("hello")))
    ...
    func deduplicates() throws {
        ...
        store.capture(item("hello", at: early))
        store.capture(item("world", at: early.addingTimeInterval(1)))
        store.capture(item("hello", at: late))
    ...
    func pinningHoists() throws {
        let store = try makeStore()
        store.capture(item("old", at: Date(timeIntervalSince1970: 1000)))
        store.capture(item("new", at: Date(timeIntervalSince1970: 2000)))
    ...
    func deletesEntry() throws {
        let store = try makeStore()
        store.capture(item("keep"))
        store.capture(item("drop"))
    ...
    func retentionEvictsOnCapture() throws {
        let store = try makeStore(retention: RetentionPolicy(maximumItems: 2, maximumAge: nil))
        for index in 0..<5 {
            store.capture(item("entry \(index)", at: Date(timeIntervalSince1970: 1000 + Double(index))))
        }
        #expect(store.items.count == 2)
    ...
    func retentionAppliesOnChange() throws {
        let store = try makeStore()
        for index in 0..<4 {
            store.capture(item("entry \(index)", at: Date(timeIntervalSince1970: 1000 + Double(index))))
        }
    ...
    func recordsSource() throws {
        let store = try makeStore()
        store.capture(item("hello", source: "com.apple.Safari"))
