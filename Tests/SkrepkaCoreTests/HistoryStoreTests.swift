// Exercises the SwiftData store. Not recreated for Linux: Phase 3's
// HistoryStoringTests runs the same assertions against every conformance. What
// the off-actor detail pass puts on a row is in HistoryStoreDetailsTests.
    ...
    struct HistoryStoreTests {
        ...
        func retentionEvictsOnCapture() async throws {
            ...
            for index in 0..<5 {
                await store.capture(
                    item("entry \(index)", at: Date(timeIntervalSince1970: 1000 + Double(index))))
            }
            #expect(store.items.count == 2)
