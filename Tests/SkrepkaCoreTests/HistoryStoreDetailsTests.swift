// Exercises the SwiftData store, like HistoryStoreTests. Not recreated for
// Linux: the detail pass is AppKit-gated with ThumbnailRenderer (D-9).
    ...
    /// the preview, the measured size, and the corrections a repeat copy applies
    /// to an entry stored before either existed.
    ...
    struct HistoryStoreDetailsTests {
        ...
        func thumbnailsImageData() async throws {
            ...
            #expect(NSImage(data: try #require(store.thumbnail(for: summary.id))) != nil)
