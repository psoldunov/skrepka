...
struct HistoryStoreDetailsTests {
    ...
    func upgradesUntypedFileThatDecodes() async throws {
        // The file system reports the generic `public.data` for a file with no
        // extension, so its type cannot say. Having drawn a picture out of it
        // can, and by then the preview exists.
        let store = try makeStore()
        let directory = try Fixtures.makeDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func healsStaleFileNamesOnRepeatCopy() async throws {
        ...
        // from the pasteboard's own text, where Finder writes the display names
        // of the whole selection — three names for a copy whose one file was
        // all that was ever kept. The repeat copy is the only chance to fix it,
        // and it must fix the names in the same breath as the file list, or the
        // row ends up naming three files while saying it holds one.
        let store = try makeStore()
        let url = try Fixtures.writeTextFile(named: "a.txt")
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let payload = Fixtures.fileURLPayload(url)

        await store.capture(ClipItem(kind: .file, text: "a.txt\nb.txt\nc.txt", payload: payload))
