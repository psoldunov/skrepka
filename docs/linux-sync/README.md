...
**Amended 2026-09-22: Linux is a supported platform, not a preview.** The
project README's [Where it runs](../../README.md#where-it-runs) table is the
current account of what was tested where, and supersedes the status notes
...
What has landed is the Sway prototype for step 1, the picker's palette, rows and
key map, [OQ-12](open-questions.md#oq-12)'s portable path IR, the daemon
connection, the global shortcut, the tray, and the devices half of Settings —
together as one desktop app, `skrepka-gui`, whose Settings pairs, unpairs and
sets live clipboard per device; what has not is thumbnails of copied image
